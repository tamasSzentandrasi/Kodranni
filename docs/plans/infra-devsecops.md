...
6. **Linux dist** — see [`storyteller-host.md`](./storyteller-host.md). Product verbs `kodranni` / `start` / `stop`; desk Discord picker; optional `kodranni-table.service` (not enabled by install); tarball with vendored Node. **Verify-ready** — [manual-test-plan.md](./manual-test-plan.md).
