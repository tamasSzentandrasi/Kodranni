...
| **V16** | Factions ≠ hierarchy axes. `labelIds[]` on every person (kin, NPC, outsider); multi-faction. Tags = ST labels. `LabelGroup` → viewer views (which marks paint). Axis heads focus a column. Do not dim the whole nave (`data-preview-faction` is gone). |
| **V17** | Outsiders = porch until inducted → Outcast. Optional `characterSlug` / sheet so ST rolls bind a slug. |
| **V18** | Find: player search. Query matches **name and label names**. Facets compose: OR within a group (axes, tiers, kinds, factions, tags), AND across groups. Faction/tag filters belong in Find (not exclusive radios). Index: kin, outsiders, ruler, pending. Hide/show: iron/glass; persist `kod-hall:{slug}`; phone default closed. Keyboard: `/` or `f` focuses; Esc clears then hides; arrows in hits; Enter opens. |
