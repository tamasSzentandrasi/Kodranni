...
export function boot(_sidebarIconMap?: SidebarIconMap): void {
	...
	function setupHierarchyBoard() {
		document.querySelectorAll('[data-widget="hierarchy-board"]').forEach((root) => {
			...
			root.querySelector('.kod-hier-links')?.remove();
			...
			function personNodes() {
				return root.querySelectorAll(
					'.kod-hier__pick[data-person], .member[data-person], .kod-chip[data-person]',
				);
			}

			function setPerson(id) {
				if (id) root.dataset.person = id;
				else root.removeAttribute('data-person');
				personNodes().forEach((chip) => {
					const on = Boolean(id) && chip.dataset.person === id;
					chip.setAttribute('aria-pressed', on ? 'true' : 'false');
					chip.classList.toggle('is-lit', on);
					if (on) chip.setAttribute('data-lit', '');
					else chip.removeAttribute('data-lit');
				});
				root.querySelectorAll('.kod-hier-rungs li').forEach((li) => {
					const hit =
						Boolean(id) &&
						li.querySelector(`.member[data-person="${id}"], .kod-chip[data-person="${id}"]`);
