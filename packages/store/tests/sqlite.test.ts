...
describe('sqlite store', () => {
  it('migrates, seeds, and redacts public snapshot without members', () => {
    ...
    const torvaldRec = store.getCharacterBySlug('tomaso')!;
    store.putCharacter({
      ...torvaldRec,
      player: {
        platform: 'discord',
    ...
    const torvald = snap.characters.find((c) => c.slug === 'tomaso')!;
    expect(torvald.name).toBe('Tomaso Fero');
    // max(Str2,Dex2)+Int3+Auth1 = 6; Exertion max Res2+Con2+Cha1 = 5
    expect(torvald.echoCapacity).toBe(6);
    expect(torvald.exertion.max).toBe(5);
    expect(torvald.echoCapacity).not.toBe(torvald.exertion.max);
    const leifr = snap.characters.find((c) => c.slug === 'jakov')!;
    // max(3,2)+2+2 = 7; Exertion max 2+2+1 = 5; echoes 2+1+2 = 5
    expect(leifr.echoCapacity).toBe(7);
    expect(leifr.exertion.max).toBe(5);
    expect(leifr.echoWeight).toBe(5);
    const json = JSON.stringify(snap);
    expect(json).not.toContain('123456789012345678');
    expect(json).not.toContain('demo-discord-jakov');
    ...
    expect(again.getCharacterBySlug('tomaso')?.exertion.current).toBe(4);
    again.close();
  });
  ...
  it('keeps drafts in listCharacters but omits them from the public snapshot', () => {
    ...
    seedDemoCampaign(store);
    const torvald = store.getCharacterBySlug('tomaso')!;
    store.putCharacter({
      ...torvald,
      id: 'draft-1',
      slug: 'mara-reed',
  ...
  it('repopulates a campaign from a public snapshot under a new campaign slug', () => {
    ...
    expect(dest.getCharacterBySlug('tomaso')?.name).toMatch(/Tomaso/i);
